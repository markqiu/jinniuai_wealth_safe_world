...
def split_data(data):
    ...
    for i in range(len(data)):
        index = data.index[i]
        row = data.iloc[i]
        category_data.append(index)
        values.append(row[["open","close","low","high", "volume"]].values.tolist())
        volumes.append([i, row["volume"],1 if row["open"] > row["close"] else -1])
...
def draw_charts(series_name, chart_data):
